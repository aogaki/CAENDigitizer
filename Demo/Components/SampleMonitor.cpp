      ...
      fServ(nullptr)
{
  ...
  std::cout << "Number of Samples" << kNSamples << std::endl;
  // Set InPort buffers
  ...
  for (uint i = 0; i < kNCh; i++) {
    ...
    fHis[i]->SetXTitle("[keV]");

    fHisInput[i] =
        new TH2D(Form("HisInput%02d", i), "Integral of input signal",
                 kNSamples / 10, 0.5, kNSamples + 0.5, 1800, 0.5, 18000.5);

    fGr[i] = new TGraph();
    for (auto j = 0; j < kNSamples; j++) fGr[i]->SetPoint(j, j + 1, 8000);
...
void SampleMonitor::FitHis()
{
  ...
  for (auto iCh = 0; iCh <= 0; iCh++) {
    ...
    if (fFitFnc[iCh] == nullptr) {
      ...
      fFitFnc[iCh]->SetParName(1, "#it{#mu}");
      fFitFnc[iCh]->SetParName(2, "#it{#sigma}");
    } else {
...
void SampleMonitor::UploadResults()
{
  ...
  auto gausFnc = new TF1("gausFnc", "gaus");
  gausFnc->SetLineColor(kGreen);
  gausFnc->SetRange(mean - 5 * sigma, mean + 5 * sigma);
  gausFnc->SetParameters(fFitFnc[0]->GetParameter(0), mean, sigma);

  auto bg =
      new TF1("Background", fitFnc, mean - 5 * sigma, mean + 5 * sigma, 7);
  bg->SetLineColor(kCyan);
  bg->SetParameters(0., fFitFnc[0]->GetParameter(1),
                    fFitFnc[0]->GetParameter(2), fFitFnc[0]->GetParameter(3),
                    fFitFnc[0]->GetParameter(4), fFitFnc[0]->GetParameter(5),
                    fFitFnc[0]->GetParameter(6));
  ...
  if (fFitCan) {  // Too Long?
    fHis[0]->GetXaxis()->SetRangeUser(mean - 5 * sigma, mean + 5 * sigma);
    fFitCan->cd();
    fHis[0]->Draw();
    gausFnc->Draw("SAME");
    bg->Draw("SAME");
    fFitCan->Print(imagePath, "jpg");
  }
  ...
  buf << "title" << Form("%2.3f", mean / 1000.) << "content"
